...
The `firmwareversion` JSON-RPC property of the DeviceInfo plugin currently returns `imagename`, `sdk`, `mediarite`, `yocto`, and `pdri` fields. The `imagename` value (read from `/version.txt`) encodes the rdk version as a segment in the image name string (e.g. `ELTE11MWR_8.6p1s2_PROD` contains `8.6p1s2`).
...
Multi-dot numeric versions like `8.5.3.7B1` or `8.7.1.0` do NOT represent rdk versions and must return the default `"0.0"`.

## Goals / Non-Goals

**Goals:**
- Expose the `rdk` version string in the `firmwareversion` response.
- Parse `rdk` from the already-read `imagename` — no new file reads.
...
**Non-Goals:**
- Caching or pre-computing rdk version at plugin startup.
- Supporting image name formats not present in the field.
- Exposing rdk version as a standalone property.

## Decisions

### D1: Parse from `imagename`, not a separate file

**Decision**: Extract `rdk` from the `imagename` string already read from `/version.txt`.

**Rationale**: The rdk version is encoded in the image name by convention. Using `imagename` as the source is reliable, requires no new I/O, and is consistent with how `releaseversion` is also derived from `imagename`.
...
| `FirmwareversionInfo` struct ABI change | `rdk` is added as a new string member; COM-RPC clients using the interface will need to be rebuilt — this is expected for any interface struct change. |

## Migration Plan

1. Add `rdk` member to `FirmwareversionInfo` in `IDeviceInfo.h`.
2. Update `DeviceInfoImplementation::FirmwareVersion()` to populate `rdk` via regex on `imagename`.
